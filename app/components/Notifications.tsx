...
function getLink(n: Notification): string | null {
  ...
  if (relatedid) {
    ...
    if (relatedtype === "conversation") return `/chat?conversation=${relatedid}`;
  }
...
function NIcon({ type }: { type: string }) {
  ...
  return (
    <span className={`inline-flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full ${c.bg}`}>
      <svg className={`h-4 w-4 ${c.fg}`} fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24">
...
export default function Notifications({ userId }: NotificationsProps) {
  ...
  const fetchNotifications = useCallback(async (lim: number) => {
    try {
      setLoading(true);
  ...
  useEffect(() => { if (isOpen) fetchNotifications(limit); }, [isOpen]);
  ...
  const dismiss = useCallback(async (e: React.MouseEvent, id: number) => {
    e.stopPropagation();
    try {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <>
          <div className="fixed inset-0 z-40 bg-black/20 backdrop-blur-[2px] md:hidden" onClick={() => setIsOpen(false)} />
          <div
          ...
          >
            <div className="flex flex-shrink-0 items-center justify-between border-b border-gray-100 px-4 py-3">
              ...
              <div className="flex items-center gap-1">
                {unreadCount > 0 && (
                  <button onClick={markAllAsRead} className="rounded px-2 py-1 text-[11px] font-medium text-emerald-700 hover:bg-emerald-50 transition-colors">
                    Mark all read
                  </button>
                )}
                <button onClick={() => setIsOpen(false)} aria-label="Close" className="flex h-7 w-7 items-center justify-center rounded text-gray-400 hover:bg-gray-100 hover:text-gray-700 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain">
              {loading && notifications.length === 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-14 px-6 text-center">
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    {filter === "unread" ? "You are all caught up." : "Activity from your listings and purchases will appear here."}
                  </p>
                </div>
              ) : (
              ...
              {hasMore && !loading && (
                <div className="border-t border-gray-100 px-4 py-3 text-center">
                  <button onClick={loadMore} className="text-xs font-medium text-emerald-700 hover:text-emerald-800 transition-colors">
                    Load more
                  </button>
                </div>
              )}
