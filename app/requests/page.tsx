...
function RequestCard({
  ...
  const isLocked = isInProgress || isFulfilled;

  return (
    <article className={`bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden flex flex-col group${isFulfilled ? " opacity-80" : ""}`}>
      {/* Top accent bar */}
      <div
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-1">
        ...
        <div className="flex items-center gap-2.5 mb-3">
          ...
          {isFulfilled ? (
            <span className="flex-shrink-0 inline-flex items-center gap-1 text-xs font-bold bg-emerald-100 text-emerald-700 px-2.5 py-0.5 rounded-full uppercase tracking-wide">
            ...
            <span className="flex-shrink-0 inline-flex items-center gap-1 text-xs font-bold bg-amber-100 text-amber-700 px-2.5 py-0.5 rounded-full uppercase tracking-wide">
              <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="flex items-center justify-between pt-3 border-t border-gray-50">
          ...
          {isLocked ? (
            isFulfilled ? (
              <span className="inline-flex items-center gap-1 text-xs font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1.5 rounded-lg">
              ...
              <span className="inline-flex items-center gap-1 text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-1.5 rounded-lg">
                <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" />
                </svg>
                Locked
        ...
        {/* Status banners for in_progress / fulfilled */}
        {isInProgress && (
          <div className="mt-3 flex items-start gap-2 px-3 py-2.5 rounded-xl bg-amber-50 border border-amber-200">
            ...
            <div>
              <p className="text-xs font-semibold text-amber-800 leading-snug">Payment in progress</p>
              <p className="text-xs text-amber-600 mt-0.5">
                {isOwner
                  ? "Your payment is being processed. Check your Orders for the latest update."
                  : "A seller is completing this request. No further responses are accepted at this time."}
              </p>
            </div>
          </div>
        )}
        {isFulfilled && (
          <div className="mt-3 flex items-start gap-2 px-3 py-2.5 rounded-xl bg-emerald-50 border border-emerald-200">
            <svg className="w-4 h-4 text-emerald-500 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
            </svg>
            <div>
              <p className="text-xs font-semibold text-emerald-800 leading-snug">Request fulfilled</p>
              <p className="text-xs text-emerald-600 mt-0.5">
                {isOwner
                  ? "Your request was successfully fulfilled. The transaction is complete."
                  : "This request has been fulfilled and is no longer accepting responses."}
              </p>
            </div>
          </div>
        )}

        {/* Expiry hint — only shown for open requests */}
        {!isLocked && expiresIn <= 5 && (
