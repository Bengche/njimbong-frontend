...
export default function SafetyTrustPage() {
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-6 sm:px-6 sm:py-10 lg:py-14">
        ...
        <div className="rounded-3xl overflow-hidden bg-gradient-to-br from-emerald-700 to-emerald-900 shadow-xl">
          <div className="px-6 py-8 sm:px-10 sm:py-10">
            <div className="flex items-start gap-4 mb-6">
              ...
              <div>
                <h2 id="fonlok-heading" className="text-2xl font-bold text-white">Fonlok Escrow Payments</h2>
                <p className="text-emerald-200 text-sm mt-1">Your money is protected on every XAF transaction</p>
              </div>
            </div>
            ...
            <p className="text-emerald-100 leading-relaxed mb-8 text-sm sm:text-base">
              ...
              dedicated escrow payment platform — to protect both buyers and sellers
              on every transaction. When you pay via Fonlok, your MoMo funds are
              held securely and only released when you confirm you have received your
              item in the expected condition.
            </p>
            ...
            <div className="grid gap-4 sm:grid-cols-2 mb-8">
              {[
                ...
                <div key={col.title} className="rounded-2xl bg-white/10 border border-white/10 px-5 py-5">
                  <p className="text-lg mb-3">{col.icon} <span className="font-bold text-white text-sm">{col.title}</span></p>
                  <ul className="space-y-2.5">
                    {col.points.map((point) => (
                      <li key={point} className="flex items-start gap-2 text-xs text-emerald-100">
            ...
            {/* Step-by-step */}
            <h3 className="text-white font-bold mb-4">How an Escrow Payment Works — Step by Step</h3>
            <div className="space-y-3">
              {[
                { n: "01", label: "Buyer initiates escrow payment", detail: "Select \"Buy Securely via Fonlok Escrow\" on a listing and enter your MoMo number. The payment prompt is sent to your phone." },
                { n: "02", label: "Funds locked in escrow vault", detail: "Your MoMo payment is deducted and locked inside the Fonlok escrow vault. The seller is notified that funds are secured." },
                { n: "03", label: "Seller prepares and delivers the item", detail: "With payment guaranteed, the seller ships the item or arranges a safe in-person handover." },
                { n: "04", label: "Buyer inspects and confirms", detail: "Once you receive the item, inspect it carefully. If satisfied, confirm delivery on Njimbong and funds are instantly released to the seller." },
                { n: "05", label: "Dispute? Open a case", detail: "If the item is not as described, wrong, or never arrives, open a dispute. Our moderation team reviews evidence from both parties and mediates a fair resolution." },
              ].map((s) => (
                <div key={s.n} className="flex gap-4 items-start rounded-xl bg-white/8 border border-white/10 px-4 py-3.5">
                  <span className="text-xs font-black text-emerald-300 mt-0.5 flex-shrink-0 w-6">{s.n}</span>
                  <div>
                    <p className="text-sm font-semibold text-white">{s.label}</p>
                    <p className="text-xs text-emerald-200 mt-0.5 leading-relaxed">{s.detail}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <a href="/signup" className="inline-flex items-center justify-center rounded-xl bg-white text-emerald-800 font-semibold px-6 py-3 text-sm hover:bg-emerald-50 transition-colors shadow">
                Start Buying Safely
              </a>
              <a href="/browse" className="inline-flex items-center justify-center rounded-xl border border-white/30 text-white font-semibold px-6 py-3 text-sm hover:bg-white/10 transition-colors">
                Browse Listings
              </a>
            </div>
          </div>
            ...
            <div className="grid grid-cols-3 gap-4 text-center">
              {[
                ...
                <div key={s.l}>
                  ...
                  <p className="text-xs text-emerald-300 mt-0.5 leading-tight">{s.l}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── OTHER PROTECTIONS ────────────────────────────────────── */}
      <section className="mt-10 grid gap-5 sm:grid-cols-2" aria-labelledby="other-protections">
        <h2 id="other-protections" className="sm:col-span-2 text-xl font-bold text-gray-900">
      ...
      <Card className="mt-8">
        <div className="flex flex-col sm:flex-row sm:items-center gap-5">
          <div className="flex-1">
            <h2 className="text-base font-bold text-gray-900">Need help or want to report an issue?</h2>
            <p className="mt-1.5 text-sm text-gray-600 leading-relaxed">
              Our support team responds as quickly as possible. Whether you have a
              dispute, spotted a suspicious listing, or have a question about an
              escrow payment — we are here.
            </p>
          </div>
          <div className="flex flex-col gap-2 sm:items-end flex-shrink-0">
            ...
            <a href="/terms-privacy" className="text-xs text-emerald-700 hover:underline font-medium">
